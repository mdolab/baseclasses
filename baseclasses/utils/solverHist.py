...
class SolverHistory(object):
    """
    The SolverHistory class stores various useful values during a nonlinear solution in a nested dictionary structure, by
    ...
    def __init__(self):
        ...
        length = int(1e4)  # TODO: Remove need for this
    ...
    def addUserFuncVars(self, Solver, UserFunc, printFuncs=True):
        """Add the variables generated by a user supplied function to those tracked by the SolverHistory object"""
        ...
        for key, val in data.items():
            if key not in self.hist["nameFormat"]:
                ...
                if isinstance(printFuncs, bool):
                    printVar = printFuncs
                elif isinstance(printFuncs, list):
                    printVar = key in printFuncs
                self.addField(Name=key, Type=type(val), Print=printVar)
            else:
                warn(
